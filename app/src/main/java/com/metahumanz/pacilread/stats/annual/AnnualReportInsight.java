...
public final class AnnualReportInsight {
    ...
    private static String globalSentence(AnnualReportData report) {
        ...
        if (hasText(report.topTag) && hasText(report.topBook)) {
            return "你今年最常读“" + trimForInsight(report.topTag, 10) + "”，代表书是《"
                    + trimForInsight(report.topBook, 14) + "》。";
        }
        if (hasText(report.topAuthor)) {
            return "今年的阅读偏好很清楚，常读作者是 "
                    + trimForInsight(report.topAuthor, 14) + "。";
        }
        if (hasText(report.topBook)) {
            return "《" + trimForInsight(report.topBook, 16) + "》占据了今年最多阅读时间。";
        }
        if (report.activeMonths >= 8 && report.readingDays >= 30) {
            return "全年有 " + report.activeMonths + " 个月留下阅读记录，累计 "
                    + report.readingDays + " 个阅读日。";
        }
        if (report.totalSeconds > 0 && report.readingDays > 0) {
            return "你今年读了 " + ReadingStatsUtils.formatDuration(report.totalSeconds)
                    + "，分布在 " + report.readingDays + " 个阅读日。";
        }
        if (report.totalChars > 0) {
            return "你今年读过 " + formatNumber(report.totalChars) + " 字，留下了可见的阅读记录。";
        }
        return "你今年留下 " + report.readingDays + " 个阅读日。";
    }
    ...
    private static String bookSentence(AnnualReportData report) {
        ...
        if (hasText(report.statusText) && report.totalSeconds > 0) {
            return "这本书状态是“" + report.statusText + "”，今年投入 "
                    + ReadingStatsUtils.formatDuration(report.totalSeconds) + "。";
        }
        if (report.totalSeconds > 0 && report.readingDays > 0) {
            return "这本书今年读了 " + ReadingStatsUtils.formatDuration(report.totalSeconds)
                    + "，覆盖 " + report.readingDays + " 个阅读日。";
        }
        if (report.totalChars > 0) {
            return "这本书今年读过 " + formatNumber(report.totalChars) + " 字。";
        }
        return "这本书今年留下 " + report.readingDays + " 个阅读日。";
    }
    ...
    private static boolean isStrongPeakMonth(AnnualReportData report) {
        if (report == null || report.peakMonth <= 0 || report.peakMonthSeconds <= 0 || report.totalSeconds <= 0) {
            return false;
        }
